...
path <- system.file("extdata", "vignette_MGnifyR_long.rds", package = "MGnifyR")
vignette_MGnifyR_long <- readRDS(path)
```
...
northpolar <- vignette_MGnifyR_long[["northpolar"]]
```
...
```{r search_studies_accession1, eval=FALSE}
study_samples <- doQuery(mg, "studies", accession="MGYS00002891")
```

```{r search_studies_accession2, eval=TRUE, include=FALSE}
study_samples <- vignette_MGnifyR_long[["study_samples"]]
```

```{r search_studies_accession3}
study_samples
```
...
analyses_accessions <- vignette_MGnifyR_long[["analyses_accessions"]]
```
...
analyses_metadata <- vignette_MGnifyR_long[["analyses_metadata"]]
```
...
```{r get_treese1, eval=FALSE}
tse <- getResult(mg, accession = analyses_accessions, get.func = FALSE)
```

```{r get_treese2, eval=TRUE, include=FALSE}
tse <- vignette_MGnifyR_long[["tse"]]
```
...
```{r get_analyses2, eval=TRUE, include=FALSE}
all_accessions <- vignette_MGnifyR_long[["all_accessions"]]
```
...
```{r get_new_metadata1, eval=FALSE}
full_metadata <- getMetadata(mg, all_accessions)
```

```{r get_new_metadata2, eval=TRUE, include=FALSE}
full_metadata <- vignette_MGnifyR_long[["full_metadata"]]
```
...
kegg <- vignette_MGnifyR_long[["kegg"]]
```
...
dl_urls <- vignette_MGnifyR_long[["dl_urls"]]
```
...
```{r get_genome_urls1, eval=FALSE}
genome_urls <- searchFile(mg, "MGYG000433953", type = "genomes")
```

```{r get_genome_urls2, eval=TRUE, include=FALSE}
genome_urls <- vignette_MGnifyR_long[["genome_urls"]]
```
...
```{r get_files2, eval=TRUE, include=FALSE}
cached_location <- vignette_MGnifyR_long[["cached_location"]]
```
...
```{r download_with_read1, eval=FALSE}
...
```{r do_download_with_read2, eval=TRUE, include=FALSE}
amoC_seq_counts <- vignette_MGnifyR_long[["amoC_seq_counts"]]
```


```{r download_with_read3}
amoC_seq_counts
```
