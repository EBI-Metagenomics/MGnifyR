...
into a single `TreeSummarizedExperiment` (`TreeSE`) object. TreeSE is becoming
...
For metagenomic samples, the result is `MultiAssayExperiment` (`MAE`) which
...
`getFile()` as the `read_func` argument. The function in question should
