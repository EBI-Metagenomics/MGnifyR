...
Other general parameters include `use.cache` and `max.hits`. 
...
northpolar <- doQuery(
    ...
    biome_name="Soil", instrument_platform = "Illumina", use.cache = FALSE)
northpolar[1:5,]
```
...
study_samples <- doQuery(
    mg, "studies", accession="MGYS00002891", use.cache=TRUE)
study_samples
```
...
`use.cache=FALSE` for `doQuery` calls, to ensure queries are actually returning
...
```{r get_mae}
mae <- getResult(
    mg, all_accessions, bulk.dl = TRUE, use.cache = TRUE)
mae
```
...
`use.cache=FALSE`) analysis of a large number of datasets may be possible with
