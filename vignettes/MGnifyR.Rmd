...
##Differential expression using MASS and glm.nb
...
getAntiMicroSeqs <- function(fname){
  sequences <- readAAStringSet(fname)
  antimicro <- grepl("PF04896", names(sequences))
  as.data.frame(as.list(table(as.character(sequences[antimicro]))))
}

#Do the download/processing
antimicro_seq_counts <- llply(urls$download_url[1:5],
                              function(x) mgnify_download(mg, x, read_func = getAntiMicroSeqs),
                              .progress = "text")

results <- rbind.fill(antimicro_seq_counts)
