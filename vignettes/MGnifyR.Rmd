...
Best practice methods for the analysis of such functional data are still under development, but simple exploratory analysis may reveal interesting insights. For instance, the code below performs a simple "read depth" normalization on the returned `go-slim` dataset, followed by NMDS dimensional reduction and plotting. Rather than acting as any sort of recommended analysis, the intention is to simply demonstrate the ease with which the MGnify data is manipulizable ...??...
...
```
From the ordination plot, we see a clear delineation between the gut and marine samples which, while unsurprising, is reassuring. Taking the analysis further we might be interested in finding out if any particular GO terms are over or under represented between biome types or studies. Since we are essentially working with count data, 
...
#Just download a single accession for demonstration, specifying a read_func
antimicro_seq_counts <- mgnify_download(mg, urls$download_urls , read_func = getAntiMicroSeqs)

antimicro_seq_counts %>% head
